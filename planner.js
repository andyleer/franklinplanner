...
//
// All per-user scoping is handled on the server via session/login.
...
(function () {
    ...
    async function saveEntry() {
        ...
        try {
            ...
            if (json && (json.status === "ok" || json.status === "saved" || json.id)) {
                ...
                // POST succeeded but no special status – still show Saved
                showSavedIndicator();
            }
        } catch (err) {
    ...
    async function loadEntry(dateStr) {
        ...
        try {
            const res = await fetch(`/api/day/${date}`);
            if (!res.ok) {
                // No entry yet for this date – just blank defaults
    ...
    function renderMiniCalendar(containerId, year, month) {
        ...
        const shownDate = new Date(year, month, 1); // JS will normalize m-1, m+1, etc.
    ...
    /* =====================================================================
       7. LAYOUT TOGGLES (stacked / phone view)
       ===================================================================== */
    ...
    function setupPhoneViewToggle() {
        ...
        phoneBtn.addEventListener("click", () => {
            ...
            body.dataset.phoneManual = "1"; // mark manual override

            phoneBtn.textContent = isPhone ? "Exit Phone View" : "Phone View";

            // In phone view, force stacked layout (looks better on narrow screens)
            if (spread && isPhone) {
    ...
    function applyAutoPhoneView() {
        const body = document.body;

        // If user already manually chose a mode, don't auto-switch.
        if (body.dataset.phoneManual === "1") return;
        ...
        if (isSmallScreen) {
            ...
            const phoneBtn = document.getElementById("phoneViewBtn");
            if (phoneBtn) phoneBtn.textContent = "Exit Phone View";
        } else {
            body.classList.remove("phone-view");
            const phoneBtn = document.getElementById("phoneViewBtn");
            if (phoneBtn) phoneBtn.textContent = "Phone View";
        }
    }
    ...
    function initPlanner() {
        ...
        // Update auto phone view on resize, unless user manually changed it
        ...
        // Ensure some blank rows exist before loading
        const list = document.getElementById("task-list");
        if (list && list.children.length === 0) {
            for (let i = 0; i < 6; i++) addBlankTask();
        }

        // Finally load data for the active date from the server
